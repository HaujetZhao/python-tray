...
class _TraySystem:
    def __init__(self):
        ...
        # 获取主程序名称
        self.title = os.path.basename(sys.argv[0]) or "Console App"
    ...
    def start(self):
        """启动所有线程"""
        # 1. 托盘图标线程
        t_tray = threading.Thread(target=self.icon.run, daemon=True)
        t_tray.start()
...
def enable_min_to_tray():
    ...
    with _lock:
        ...
        _tray_instance = _TraySystem()
        _tray_instance.start()

# --- 模块导入时自动执行 ---
# 只要 import 这个文件，就会自动启动托盘功能
enable_min_to_tray()
